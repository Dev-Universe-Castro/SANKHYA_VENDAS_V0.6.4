...
export default function ProductsTable() {
  ...
  const loadProducts = async () => {
    console.log('📍 loadProducts chamado - início')
    try {
      setIsLoading(true)
      ...
      if (hasSearch && searchQuery.length >= 2) {
        ...
        console.log('🔄 Usando busca rápida:', url)
      } else {
        ...
        console.log('🔄 Usando busca completa:', url)
      }
      
      const controller = new AbortController()
      const timeoutId = setTimeout(() => {
        console.log('⏰ Timeout alcançado após 60s')
        controller.abort()
      }, 60000)
      ...
      console.log('📦 Produtos carregados:', data.produtos?.length || 0, 'Total:', data.total)
